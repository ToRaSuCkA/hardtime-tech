...
function toRow(r: EolResult): string[] {
  return [
    ...
    r.eosaleDate ?? '',
    r.eosupportDate ?? '',
    (r.status === 'eol' || r.status === 'end-of-sale') ? '' : (r.replacementCostSame ?? ''),
...
export async function toPDF(results: EolResult[]): Promise<Blob> {
  ...
  autoTable(doc, {
    startY: 28,
    head: [['Product', 'Status', 'Sec. Patch End', 'Support End', 'Same Cost', 'Priority', 'Rec. Replacement', 'Rec. Cost', 'Source']],
    body: results.map(r => [
      r.productName,
      r.status,
      r.eolDate ?? '—',
